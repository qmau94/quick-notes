...
|:----|:----|
|+| Indicates a number is positive, although numbers are assumed to be positive in Java unless accompanied by a negative unary operatpr|
...
### Equality Operators
- The equality operators are used in one of three scenariors:
...
- It is possible to add optional labels to control and block stutures. However, it is rarely considered good coding practice to do so.
...
## Summary
- For statement, there are two types of control strutures:
  - Decision-making control strutures: if-then, if-then-else, and switch  
  - Repetition control strutures: for, for-each, while, and do-while.
- Most of these strutures require the evaluation of a particular boolean expression either for branching decisions or once per repetition. The switch statement is the only one that supports a variety of data types, including String variables as of Java 7.
- For-each statement does not need to explicity write a boolean expression, since the compiler builds them implicitly. For clarity, we referred to an enhanced for loop as a for-each loop, but syntactically they are written as a for statement.
...
- Understand how break and continue can chage flow control.
